...
class ProviderController extends BaseController {
  ...
  ethRpc = (req, forceChainServerId?: string) => {
    ...
    let chainServerId = CHAINS[CHAINS_ENUM.PULSE].serverId;
    if (site) {
  ...
  ethSign = () => {
    throw new Error(
      "Signing with 'eth_sign' can lead to asset loss. For your safety, PulseWallet does not support this method."
    );
  };
  ...
  walletAddEthereumChain = ({
    ...
    if (!chain) {
      throw new Error('This chain is not supported by PulseWallet yet.');
    }
