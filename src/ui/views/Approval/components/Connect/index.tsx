...
const Connect = ({ params: { icon, origin } }: ConnectProps) => {
  ...
  const [defaultChain, setDefaultChain] = useState(CHAINS_ENUM.PULSE);
  ...
  const init = async () => {
    ...
    let defaultChain = CHAINS_ENUM.PULSE;
    ...
    queue.add(async () => {
      try {
        ...
        defaultChain = targetChain ? targetChain.enum : CHAINS_ENUM.PULSE;
      } catch (e) {
        console.log(e);
      }
    });
